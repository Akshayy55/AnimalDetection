...
def plot_points_on_bird_eye_view(frame, pedestrian_boxes, M, scale_w, scale_h):
    ...
    for i in range(len(pedestrian_boxes)):
        ...
        mid_points.append([mid_point_x, mid_point_y])
        
        print("mid_point_x, mid_point_y", mid_point_x, mid_point_y)
    ...
    return warped_pts, bird_image, mid_points
...
def plot_pedestrian_boxes_on_image(frame, pedestrian_boxes, warped_pts, bird_image, d_thresh, fps):
    
    
    six_feet_violations, ten_feet_violations, pairs, total_six_feet_violations = check_threshold_violation(
            warped_pts, bird_image, d_thresh, fps
        )
    ...
    # color_10 = (80, 172, 110)
    color_node_violation = (255,255,255)
    ...
    for i in range(len(pedestrian_boxes)):
        ...
        # print("inside condtion", total_six_feet_violations)
        if total_six_feet_violations > 0 :
            

            frame_with_boxes = cv2.rectangle(frame, pt1, pt2, color_node_violation, thickness)
...
def plot_pedestrian_boxes_on_image(frame, pedestrian_boxes, total_six_feet_violations,six_feet_violations, fps, count):
    ...
    for i in range(len(pedestrian_boxes)):
        ...
        # print("total_six_feet_violations", int(total_six_feet_violations))
        
        # if int(total_six_feet_violations) = 
        
        
        # # if count == 1 :
        if int(total_six_feet_violations) == count:
            
            # print("if condition total_six_feet_violations", int(total_six_feet_violations))
            
            
            # print("if..... count", count)
            # # count = count + 1 
            # print(int(total_six_feet_violations))
            # print("---------------------")
            ...
            # print("else values......", int(total_six_feet_violations))
        count = count + 1 
...
def check_threshold_violation(warped_points, bird_image, d_thresh, fps):
    p = np.array(warped_points)
    dist_condensed = pdist(p)
    dist = squareform(dist_condensed)
    ...
    print("six_feet_violations, ten_feet_violations, total_pairs")
    print(six_feet_violations, ten_feet_violations, total_pairs)
    
    total_six_feet_violations = 0
    total_six_feet_violations += six_feet_violations / fps
    
    print("total_six_feet_violations first", total_six_feet_violations)
    
    # total_six_feet_violations = int(total_six_feet_violations)
    
    # print("total_six_feet_violations second", total_six_feet_violations)
    
    return six_feet_violations, ten_feet_violations, total_pairs, total_six_feet_violations
